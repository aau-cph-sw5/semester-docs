...
| **Data readiness** | Structured station-layout data including concourse areas. BLE beacons installed at all stations; train beacons under consideration. |
...
**24 items.** Priority: 13 Must, 9 Should, 2 Could. Readiness: 14 ready, 2 needing refinement, 8 blocked on Metro input. Size: 3×S, 13×M, 8×L.
...
|---|---|---|---|---|---|---|
| `MET-B-001` | Evidence rule decision: is presence asserted by the system or confirmed by the steward? | B-EP1 | S | Must | Blocked | 1 |
| `MET-B-002` | Positioning interface with a simulator implementation | B-EP1 | M | Must | Ready | 1 |
| `MET-B-003` | Station reference dataset with concourse and platform areas | B-EP1 | M | Must | Blocked | 2 |
| `MET-B-004` | Presence and patrol record capture | B-EP2 | M | Must | Blocked | 2 |
...
| `MET-B-007` | Patrol completion rule and hourly coverage computation | B-EP3 | L | Must | Blocked | 3 |
...
| `MET-B-010` | Train manning input behind an abstraction | B-EP4 | M | Should | Ready | 4 |
| `MET-B-011` | Manned-train percentage against the 70 percent target | B-EP4 | M | Should | Ready | 4 |
| `MET-B-012` | Real-time shift compliance dashboard | B-EP5 | L | Must | Ready | 4 |
| `MET-B-013` | Exportable per-night compliance report with traceable records | B-EP5 | L | Must | Blocked | 5 |
...
`size:S` `prio:Must` `status:Blocked` `track:cross-team` `type:spike` `sprint:1`
...
`size:M` `prio:Must` `status:Blocked` `track:backend` `type:feature` `sprint:2`
...
`size:L` `prio:Must` `status:Blocked` `track:backend` `type:feature` `sprint:3`
...
`size:M` `prio:Should` `status:Ready` `track:backend` `type:tech` `sprint:4`
...
`size:L` `prio:Must` `status:Blocked` `track:backend` `type:compliance` `sprint:5`
